...
function App() {
  ...
  const DEFAULT_PROMPT1 = "You are an expert botanist and archivist. Your task is to meticulously transcribe all text from the labels on the provided herbarium specimen image. Perform a verbatim transcription of both handwritten and printed text exactly as it appears. Rules: Do not guess, summarize, or make up information. Retain the structure and grouping of the text as much as possible. If a word or phrase is completely illegible, write '[illegible]'. If you are unsure about a specific word, denote it with a question mark like this: '[word?]'. Explicitly include any stamps, barcodes, accession numbers, or annotations you see. Do not provide extraneous descriptions or higher taxonomy; focus entirely on extracting every piece of text accurately.";
  const DEFAULT_PROMPT2 = "You are a biodiversity data steward. Your task is to structure the provided verbatim transcription of a herbarium specimen label into a valid JSON object using exclusively valid Darwin Core (DwC) terms.\n\nUse the following Darwin Core definitions to accurately map the transcribed text:\n- `dwc:scientificName`: The full scientific name on the label, including authorship if present.\n- `dwc:recordedBy`: A list of names of people who collected the specimen (the collectors).\n- `dwc:recordNumber`: The identifier given to the collection event by the collector (often the collector number appearing near their name).\n- `dwc:eventDate`: The date the specimen was collected (format as YYYY-MM-DD if unambiguous, otherwise preserve verbatim).\n- `dwc:identifiedBy`: The name(s) of the person(s) who identified the species (often marked with 'Det.').\n- `dwc:country`: The country where the specimen was collected.\n- `dwc:stateProvince`: The state, province, or major administrative region.\n- `dwc:county`: The county or secondary administrative region.\n- `dwc:locality`: The specific text describing the exact place of collection.\n- `dwc:habitat`: The environment or habitat description (e.g., 'growing in sandy soil', 'oak forest').\n- `dwc:verbatimElevation`: The original description of the elevation or altitude.\n- `dwc:catalogNumber`: The herbarium's barcode, accession number, or physical stamp identifier.\n\nStrict constraints:\n1. Output ONLY valid JSON. Do not include markdown block formatting (e.g., ```json) or any conversational text.\n2. Use ONLY the 'dwc:' prefixed terms above as keys.\n3. If a term has multiple values (e.g., multiple collectors for `dwc:recordedBy`), separate the values with a pipe character `|` (e.g., \"John Doe | Jane Smith\").\n4. If the transcription lacks data for a specific DwC term, do NOT include that key in the JSON. Do not use placeholders like \"Unknown\" or null.\n5. Do not hallucinate or add outside geographic knowledge not supported by the transcription.\n\nExpected structure:\n{\n  \"dwc:scientificName\": \"...\",\n  \"dwc:recordedBy\": \"... | ...\",\n  \"dwc:locality\": \"...\"\n}";
